...
class MigrasiResultOld extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:migrasi-result-old';
     ...
     * @var string
     */
    protected $description = 'Migrasi data dari hospital_survey_indicator_result_old ke hospital_survey_indicator_result (Langkah 2)';
    ...
    {
        $this->info('Memulai migrasi data hospital_survey_indicator_result_old (Lanjutan)...');

        // *** SESUAIKAN NAMA TABEL INI JIKA PERLU ***
        $tableNameOld = 'hospital_survey_indicator_result_old';
        $tableNameNew = 'hospital_survey_indicator_result';

        // 1. Disable foreign key checks
        Schema::disableForeignKeyConstraints();

        // 2. Ambil semua data dari tabel lama
        // Periksa apakah tabel lama ada
        if (! Schema::hasTable($tableNameOld)) {
            $this->error("Tabel '{$tableNameOld}' tidak ditemukan. Pastikan Anda sudah mengimpor .sql lama dengan nama tersebut.");
        ...
        // 3. Copy data ke tabel baru (APPEND, JANGAN TRUNCATE)
        $successCount = 0;
        $errorCount = 0;
        ...
        foreach ($oldData as $data) {
            try {
                ...
                // Cek jika error karena duplikat PK (sudah ada dari file pertama)
                if (str_contains($e->getMessage(), 'Duplicate entry')) {
                    $this->warn("\n Data ID: {$data->result_id} sudah ada, dilewati.");
                } else {
        ...
        // 4. Reset Auto-Increment counter di tabel baru
        try {
            $maxId = DB::table($tableNameNew)->max('result_id');
            $nextId = $maxId + 1;

            // Syntax untuk MySQL. Sesuaikan jika Anda menggunakan DB lain (misal: PostgreSQL)
            $dbDriver = DB::connection()->getDriverName();
            if ($dbDriver === 'mysql') {
                DB::statement("ALTER TABLE {$tableNameNew} AUTO_INCREMENT = {$nextId}");
                $this->info("Auto-increment untuk tabel '{$tableNameNew}' direset ke {$nextId}.");
            } elseif ($dbDriver === 'pgsql') {
                // Ambil nama sequence, biasanya {tabel}_{kolom}_seq
                $sequenceName = "{$tableNameNew}_result_id_seq";
                DB::statement("SELECT setval('{$sequenceName}', {$maxId}, true)");
                $this->info("Sequence untuk tabel '{$tableNameNew}' direset ke {$nextId}.");
            } else {
                $this->warn("Tidak dapat mereset auto-increment untuk driver '{$dbDriver}'. Harap lakukan secara manual.");
            }

        } catch (\Exception $e) {
            $this->error("\n Gagal mereset auto-increment. Error: ".$e->getMessage());
        }
        ...
        // 6. Summary
        $this->info('===================================================');
        $this->info('MIGRASI LANGKAH 2 SELESAI!');
        $this->info("✅ Berhasil: {$successCount} records ditambahkan");
